...
func generateHandler(buf *bytes.Buffer, op *openapi.Operation, pathParam []*openapi.Parameter) error {
	...
	isFirstArg := true
	buf.WriteString(fmt.Sprintf("err := %s(", op.OperationID))
	for _, p := range parameters {
		if !isFirstArg {
			buf.WriteString(", ")
		}
		buf.WriteString(camelCase(p.Name))
		isFirstArg = false
	}
	if op.RequestBody != nil {
		if !isFirstArg {
...
func generateLogic(buf *bytes.Buffer, op *openapi.Operation, pathParam []*openapi.Parameter) error {
	...
	if op.RequestBody != nil {
		...
		buf.WriteString(fmt.Sprintf("req input.%s", strings.Split(op.RequestBody.Ref, "/")[3]))
	}
	buf.WriteString(")")
